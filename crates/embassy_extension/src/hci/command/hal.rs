...
pub trait HalCommands {
    ...
    /// code [command disallowed](bluetooth_hci::Status::CommandDisallowed) will return.
...
impl<'buf, const N: usize> HalCommands for crate::hci::RadioCoprocessor<'buf, N> {
    type Error = ();

    fn get_firmware_revision(&mut self) -> nb::Result<(), Self::Error> {
        self.write_command(crate::hci::opcode::HAL_GET_FIRMWARE_REVISION, &[])
    }
    ...
    fn read_config_data(&mut self, param: ConfigParameter) -> nb::Result<(), Self::Error> {
        self.write_command(crate::hci::opcode::HAL_READ_CONFIG_DATA, &[param as u8])
    }
    ...
    fn set_tx_power_level(&mut self, level: PowerLevel) -> nb::Result<(), Self::Error> {
        ...
        self.write_command(crate::hci::opcode::HAL_SET_TX_POWER_LEVEL, &bytes)
    }

    fn device_standby(&mut self) -> nb::Result<(), Self::Error> {
        self.write_command(crate::hci::opcode::HAL_DEVICE_STANDBY, &[])
    }

    fn get_tx_test_packet_count(&mut self) -> nb::Result<(), Self::Error> {
        self.write_command(crate::hci::opcode::HAL_TX_TEST_PACKET_COUNT, &[])
    }
    ...
    fn start_tone(&mut self, channel: u8) -> nb::Result<(), Error<Self::Error>> {
        ...
        self.write_command(crate::hci::opcode::HAL_START_TONE, &[channel])
            .map_err(rewrap_error)
    }

    fn stop_tone(&mut self) -> nb::Result<(), Self::Error> {
        self.write_command(crate::hci::opcode::HAL_STOP_TONE, &[])
    }

    fn get_link_status(&mut self) -> nb::Result<(), Self::Error> {
        self.write_command(crate::hci::opcode::HAL_GET_LINK_STATUS, &[])
    }

    fn get_anchor_period(&mut self) -> nb::Result<(), Self::Error> {
        self.write_command(crate::hci::opcode::HAL_GET_ANCHOR_PERIOD, &[])
    }
}
...
impl ConfigData {
    ...
    pub fn public_address(addr: bluetooth_hci::BdAddr) -> ConfigDataDiversifierBuilder {
        let mut data = Self {
            offset: 0,
            length: 6,
    ...
    /// us to start with any field, and the returned builder allows only either chaining the next
    /// field or building the structure to write.
    pub fn random_address(addr: bluetooth_hci::BdAddr) -> ConfigDataDiversifierBuilder {
        let mut data = Self {
            offset: 0x2E,
            length: 6,
            value_buf: [0; Self::MAX_LENGTH],
    ...
    pub fn encryption_root(
        key: &bluetooth_hci::host::EncryptionKey,
