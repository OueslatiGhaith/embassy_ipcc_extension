...
use bluetooth_hci::types::{ConnectionInterval, ExpectedConnectionLength};
...
pub trait L2capCommands {
    ...
    /// # Generated events
    ///
    /// A [command status](bluetooth_hci::event::Event::CommandStatus) event on the receipt of the command and
...
impl<'buf, const N: usize> L2capCommands for crate::hci::RadioCoprocessor<'buf, N> {
    type Error = ();

    impl_params!(
        connection_parameter_update_request,
        ConnectionParameterUpdateRequest,
        crate::hci::opcode::L2CAP_CONN_PARAM_UPDATE_REQ
    );

    impl_params!(
        connection_parameter_update_response,
        ConnectionParameterUpdateResponse,
        crate::hci::opcode::L2CAP_CONN_PARAM_UPDATE_RESP
    );
}
