...
use bluetooth_hci::host::EventFlags;
...
pub use bluetooth_hci::types::{ConnectionInterval, ConnectionIntervalError};
pub use bluetooth_hci::{BdAddr, BdAddrType, ConnectionHandle};
...
impl TryFrom<u8> for Status {
    ...
    fn try_from(value: u8) -> Result<Self, <Self as TryFrom<u8>>::Error> {
        match value {
            ...
            _ => Err(bluetooth_hci::BadStatusError::BadValue(value)),
        }
    }
}
...
macro_rules! require_len {
    ($left:expr, $right:expr) => {
        if $left.len() != $right {
            return Err(bluetooth_hci::event::Error::BadLength($left.len(), $right));
        }
    };
}
...
impl bluetooth_hci::event::VendorEvent for Stm32Wb5xEvent {
    type Error = Stm32Wb5xError;
    type Status = Status;
    type ReturnParameters = command::ReturnParameters;

    fn new(buffer: &[u8]) -> Result<Self, bluetooth_hci::event::Error<Stm32Wb5xError>> {
        ...
        match event_code {
            ...
            0x0401 => Ok(Stm32Wb5xEvent::GapPairingComplete(to_gap_pairing_complete(buffer)?)),
            0x0402 => Ok(Stm32Wb5xEvent::GapPassKeyRequest(to_conn_handle(buffer)?)),
            0x0403 => Ok(Stm32Wb5xEvent::GapAuthorizationRequest(to_conn_handle(buffer)?)),
            ...
            0x0407 => Ok(Stm32Wb5xEvent::GapProcedureComplete(to_gap_procedure_complete(buffer)?)),
            0x0408 => {
                #[cfg(feature = "ms")]
                {
                ...
                #[cfg(not(feature = "ms"))]
                {
                    Ok(Stm32Wb5xEvent::GapReconnectionAddress(to_gap_reconnection_address(buffer)?))
                }
            }
            ...
            0x0801 => {
                Ok(Stm32Wb5xEvent::L2CapProcedureTimeout(to_l2cap_procedure_timeout(buffer)?))
            }
            0x0802 => Ok(Stm32Wb5xEvent::L2CapConnectionUpdateRequest(
                to_l2cap_connection_update_request(buffer)?,
            )),
            0x0C01 => {
                Ok(Stm32Wb5xEvent::GattAttributeModified(to_gatt_attribute_modified(buffer)?))
            }
            0x0C02 => Ok(Stm32Wb5xEvent::GattProcedureTimeout(to_conn_handle(buffer)?)),
            0x0C03 => Ok(Stm32Wb5xEvent::AttExchangeMtuResponse(to_att_exchange_mtu_resp(buffer)?)),
            ...
            0x0C06 => {
                Ok(Stm32Wb5xEvent::AttReadByTypeResponse(to_att_read_by_type_response(buffer)?))
            }
            0x0C07 => Ok(Stm32Wb5xEvent::AttReadResponse(to_att_read_response(buffer)?)),
            0x0C08 => Ok(Stm32Wb5xEvent::AttReadBlobResponse(to_att_read_response(buffer)?)),
            0x0C09 => Ok(Stm32Wb5xEvent::AttReadMultipleResponse(to_att_read_response(buffer)?)),
            0x0C0A => Ok(Stm32Wb5xEvent::AttReadByGroupTypeResponse(
                to_att_read_by_group_type_response(buffer)?,
            )),
            0x0C0C => {
                Ok(Stm32Wb5xEvent::AttPrepareWriteResponse(to_att_prepare_write_response(buffer)?))
            }
            0x0C0D => Ok(Stm32Wb5xEvent::AttExecuteWriteResponse(to_conn_handle(buffer)?)),
            ...
            0x0C10 => {
                Ok(Stm32Wb5xEvent::GattProcedureComplete(to_gatt_procedure_complete(buffer)?))
            }
            0x0C11 => Ok(Stm32Wb5xEvent::AttErrorResponse(to_att_error_response(buffer)?)),
            0x0C12 => Ok(Stm32Wb5xEvent::GattDiscoverOrReadCharacteristicByUuidResponse(
                to_attribute_value(buffer)?,
            )),
            0x0C13 => Ok(Stm32Wb5xEvent::AttWritePermitRequest(to_write_permit_request(buffer)?)),
            0x0C14 => Ok(Stm32Wb5xEvent::AttReadPermitRequest(to_att_read_permit_request(buffer)?)),
            ...
            0x0C16 => {
                #[cfg(feature = "ms")]
                {
                    Ok(Stm32Wb5xEvent::GattTxPoolAvailable(to_gatt_tx_pool_available(buffer)?))
                }

                #[cfg(not(feature = "ms"))]
                {
                    Err(bluetooth_hci::event::Error::Vendor(Stm32Wb5xError::UnknownEvent(
                        event_code,
                    )))
                }
            }
            0x0C17 => {
                #[cfg(feature = "ms")]
                {
                    Ok(Stm32Wb5xEvent::GattServerConfirmation(to_conn_handle(buffer)?))
                }
            ...
            _ => Err(bluetooth_hci::event::Error::Vendor(Stm32Wb5xError::UnknownEvent(event_code))),
        }
    }
}
...
fn to_coprocessor_ready(
    buffer: &[u8],
) -> Result<FirmwareKind, bluetooth_hci::event::Error<Stm32Wb5xError>> {
...
macro_rules! require_l2cap_event_data_len {
    ($left:expr, $right:expr) => {
        let actual = $left[4];
        if actual != $right {
            return Err(bluetooth_hci::event::Error::Vendor(Stm32Wb5xError::BadL2CapDataLength(
                actual, $right,
            )));
        }
    };
...
macro_rules! require_l2cap_len {
    ($actual:expr, $expected:expr) => {
        if $actual != $expected {
            return Err(bluetooth_hci::event::Error::Vendor(Stm32Wb5xError::BadL2CapLength(
                $actual, $expected,
            )));
        }
    };
...
fn to_l2cap_connection_update_response(
    buffer: &[u8],
) -> Result<L2CapConnectionUpdateResponse, bluetooth_hci::event::Error<Stm32Wb5xError>> {
...
fn to_l2cap_procedure_timeout(
    buffer: &[u8],
) -> Result<ConnectionHandle, bluetooth_hci::event::Error<Stm32Wb5xError>> {
...
fn to_l2cap_connection_update_request(
    buffer: &[u8],
) -> Result<L2CapConnectionUpdateRequest, bluetooth_hci::event::Error<Stm32Wb5xError>> {
...
fn to_gap_pairing_complete(
    buffer: &[u8],
) -> Result<GapPairingComplete, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    require_len!(buffer, 5);
...
pub use bluetooth_hci::event::AdvertisementEvent as GapDeviceFoundEvent;

fn to_gap_device_found(
    buffer: &[u8],
) -> Result<GapDeviceFound, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    ...
    let mut event = GapDeviceFound {
        event: buffer[2].try_into().map_err(|e| {
            if let bluetooth_hci::event::Error::BadLeAdvertisementType(code) = e {
                bluetooth_hci::event::Error::Vendor(Stm32Wb5xError::BadGapDeviceFoundEvent(code))
            } else {
                unreachable!()
            }
        })?,
        bdaddr: bluetooth_hci::to_bd_addr_type(buffer[3], addr).map_err(|e| {
            bluetooth_hci::event::Error::Vendor(Stm32Wb5xError::BadGapBdAddrType(e.0))
        })?,
        data_len,
        data_buf: [0; 31],
...
fn to_gap_procedure_complete(
    buffer: &[u8],
) -> Result<GapProcedureComplete, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    require_len_at_least!(buffer, 4);
    ...
    let procedure = match buffer[2] {
        ...
        _ => {
            return Err(bluetooth_hci::event::Error::Vendor(Stm32Wb5xError::BadGapProcedure(
                buffer[2],
            )));
        }
    };
...
fn to_gap_reconnection_address(
    buffer: &[u8],
) -> Result<BdAddr, bluetooth_hci::event::Error<Stm32Wb5xError>> {
...
fn to_gatt_attribute_modified(
    buffer: &[u8],
) -> Result<GattAttributeModified, bluetooth_hci::event::Error<Stm32Wb5xError>> {
...
fn to_att_exchange_mtu_resp(
    buffer: &[u8],
) -> Result<AttExchangeMtuResponse, bluetooth_hci::event::Error<Stm32Wb5xError>> {
        ...
        match *self {
            HandleUuidPairs::Format16(count, pairs) => {
                for handle_uuid_pair in &pairs[..count] {
                    write!(f, "{{{:?}, {:?}}}", handle_uuid_pair.handle, handle_uuid_pair.uuid)?
                }
            }
...
fn to_att_find_information_response(
    buffer: &[u8],
) -> Result<AttFindInformationResponse, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    ...
    Ok(AttFindInformationResponse {
        ...
        handle_uuid_pairs: match buffer[5] {
            1 => {
                to_handle_uuid16_pairs(&buffer[6..]).map_err(bluetooth_hci::event::Error::Vendor)?
            }
...
fn to_att_find_by_value_type_response(
    buffer: &[u8],
) -> Result<AttFindByTypeValueResponse, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    const PAIR_LEN: usize = 4;
    ...
    if pair_buffer.len() % PAIR_LEN != 0 {
        return Err(bluetooth_hci::event::Error::Vendor(Stm32Wb5xError::AttFindByTypeValuePartial));
    }
...
fn to_att_read_by_type_response(
    buffer: &[u8],
) -> Result<AttReadByTypeResponse, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    require_len_at_least!(buffer, 6);
...
fn to_att_read_response(
    buffer: &[u8],
) -> Result<AttReadResponse, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    require_len_at_least!(buffer, 5);
...
fn to_att_read_by_group_type_response(
    buffer: &[u8],
) -> Result<AttReadByGroupTypeResponse, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    require_len_at_least!(buffer, 6);
...
fn to_att_prepare_write_response(
    buffer: &[u8],
) -> Result<AttPrepareWriteResponse, bluetooth_hci::event::Error<Stm32Wb5xError>> {
...
fn to_attribute_value(
    buffer: &[u8],
) -> Result<AttributeValue, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    require_len_at_least!(buffer, 7);
...
fn to_gatt_procedure_complete(
    buffer: &[u8],
) -> Result<GattProcedureComplete, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    require_len!(buffer, 6);
...
fn to_att_error_response(
    buffer: &[u8],
) -> Result<AttErrorResponse, bluetooth_hci::event::Error<Stm32Wb5xError>> {
...
fn to_att_read_permit_request(
    buffer: &[u8],
) -> Result<AttReadPermitRequest, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    require_len!(buffer, 8);
...
fn to_att_read_multiple_permit_request(
    buffer: &[u8],
) -> Result<AttReadMultiplePermitRequest, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    require_len_at_least!(buffer, 5);
...
fn to_gatt_tx_pool_available(
    buffer: &[u8],
) -> Result<GattTxPoolAvailable, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    require_len!(buffer, 6);
...
fn to_att_prepare_write_permit_request(
    buffer: &[u8],
) -> Result<AttPrepareWritePermitRequest, bluetooth_hci::event::Error<Stm32Wb5xError>> {
    require_len_at_least!(buffer, 9);
